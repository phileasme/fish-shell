function hgrep -d "Search command history with frequency and recency ranking across multiple shells"
    ...
    set -l show_help 0
    set -l debug_mode 0
    set -l zsh_only 0  # New flag for zsh-only mode
    ...
    while test $i -le (count $argv)
        switch $argv[$i]
            ...
            case '--debug'
                set debug_mode 1
            case '--zsh-only'  # New flag for zsh-only mode
                set zsh_only 1
            case '-*'
    ...
    if test $show_help -eq 1; or test -z "$search_term"
        ...
        echo "  -d, --depth DEPTH       Number of history entries to search per shell (default: 300)"
    ...
    set -l bash_history_file ~/.bash_history
    set -l fish_history_path ~/.local/share/fish/fish_history
    ...
    if test $zsh_only -eq 0
        set -l fish_count 0
        for cmd in (history -n $history_depth | grep -i "$search_term")
    ...
    for i in (seq (count $freq_sorted_indices))
        ...
        if test $debug_mode -eq 1
            ...
            # Command with highlighting for top command
    ...
    if test (count $freq_sorted_indices) -ge 1
        ...
        echo -n $unique_commands[$top_idx] | fish_clipboard_copy
    end
end